...
st.set_page_config(page_title="Budget vert - Axe 1")

# Titre et description
st.title("🌱 Budget Vert - Axe 1 : Atténuation du changement climatique")
st.markdown("""
Cet outil utilise un **arbre de décision** basé sur l'**Annexe technique "Climat atténuation"** de l'I4CE. N'hésitez pas à consulter cette annexe pour plus de détails.

Sélectionnez une rubrique et répondez aux questions pour obtenir le classement de votre dépense.
""")

#st.markdown(" ")
st.subheader("Questionnaire")

# Arbre de décision basé sur les tableaux de synthèse des détails de l'annexe technique
arbre_decision = {
    "Bâtiment": {
        "Sous-rubrique": {
...
if rubrique in arbre_decision:
    ...
    try:
        ...
        #st.write(f"{st.session_state.classement}")
        st.write(f"**Impact :** {st.session_state.classement}")
        ...
        if (("Très favorable" in st.session_state.classement) or ("Favorable sous conditions" in st.session_state.classement)) and ("Défavorable" in st.session_state.classement):
            st.warning("⚠️ Impact global **Mixte** : Cette dépense a des impacts positifs et négatifs.")
        elif "Très favorable" in st.session_state.classement:
            st.success("✅ Impact global **Favorable** : Cette dépense réduit significativement les émissions de GES.")
        elif "Favorable sous conditions" in st.session_state.classement:
            st.success("⚠️ Impact global **Favorable sous conditions** : Cette dépense améliore la performance, mais sous conditions.")
        elif "Défavorable" in st.session_state.classement:
            st.error("❌ Impact global **Défavorable** : Cette dépense augmente les émissions de GES.")
        elif "Neutre" in st.session_state.classement:
            st.info("ℹ️ Impact global **Neutre** : Cette dépense n'a pas d'effet significatif sur les émissions.")
            ...
            st.warning("🔍 **À approfondir** : Analyse complémentaire nécessaire.")
